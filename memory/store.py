...
"""SQLite persistent storage with FTS and session linking."""
import sqlite3
import json
import time
import re
...
class MemoryStore:
    ...
    def _init_db(self):
        ...
        # FTS5 virtual table — only id (unindexed) + content
        try:
            c.execute("""
                CREATE VIRTUAL TABLE IF NOT EXISTS memories_fts
                USING fts5(id UNINDEXED, content, tokenize='porter ascii')
            ...
            # NOTE: the trigger must only reference columns that exist in
            # memories_fts — that is 'id' and 'content'.
            c.execute("""
    ...
    def _sanitize_fts_query(self, query: str) -> Optional[str]:
        """
        Turn an arbitrary string into a safe FTS5 MATCH expression.

        Strategy:
        1. Keep only alphanumeric tokens (drop punctuation, colons, etc.)
        2. Drop FTS5 reserved words (AND / OR / NOT / NEAR)
        3. Drop tokens shorter than 2 characters
        4. Join survivors with OR (broad match)
        """
        if not query or not query.strip():
            return None

        # Extract only word characters; split on anything else
        tokens = re.findall(r'\w+', query)

        fts_reserved = {'AND', 'OR', 'NOT', 'NEAR', 'and', 'or', 'not', 'near'}
        tokens = [t for t in tokens if t not in fts_reserved and len(t) >= 2]
        ...
        # Limit to first 10 tokens to keep the query short
    ...
    def save_node(self, node: MemoryNode, session_id: Optional[str] = None):
        ...
        """, (
            ...
            json.dumps(node.metadata), node.timestamp, node.last_accessed,
            json.dumps(node.connections), node.importance, node.access_count,
            session_id,
        ))
        conn.commit()
        conn.close()
    ...
    def load_nodes(self, node_ids: List[str]) -> List[MemoryNode]:
        ...
        c.execute(f"""
            ...
            FROM memories WHERE id IN ({placeholders})
        """, node_ids)
        nodes = [self._row_to_node(row) for row in c.fetchall()]
        conn.close()
        return nodes
    ...
    def search_fts(self, query: str, limit: int = 5) -> List[MemoryNode]:
        ...
        try:
            c.execute("""
                SELECT m.id, m.type, m.content, m.metadata, m.timestamp,
                       m.last_accessed, m.connections, m.importance, m.access_count
                FROM memories m
                JOIN memories_fts f ON m.id = f.id
                WHERE memories_fts MATCH ?
            ...
            print(f"FTS search failed: {e}")
            print(f"Sanitized query was: {sanitized}")
            conn.close()
            return []

    def search_similar(self, query: str, limit: int = 5) -> List[MemoryNode]:
        """Hybrid search: FTS5 first, keyword LIKE fallback."""
        fts_results = self.search_fts(query, limit=limit) if self.fts_available else []
        if len(fts_results) >= limit:
            return fts_results
        ...
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        existing_ids = {n.id for n in fts_results}
        ...
        try:
            ...
            for row in c.fetchall():
                if row[0] not in existing_ids:
                    fts_results.append(self._row_to_node(row))
        except Exception as e:
            print(f"Keyword search failed: {e}")
    ...
    def find_similar_sessions(self, task: str, limit: int = 3) -> List[Dict]:
        """Find sessions whose task text overlaps with the given task."""
        if not task or not task.strip():
            return []
        ...
        keywords = [k for k in keywords if len(k) >= 3][:8]
        if not keywords:
            conn.close()
            return []
        ...
        try:
            c.execute("PRAGMA table_info(sessions)")
            columns = {col[1] for col in c.fetchall()}
            mem_col = "memory_count" if "memory_count" in columns else "0 as memory_count"

            c.execute(f"""
                SELECT id, task, timestamp, result, duration, {mem_col}
                FROM sessions
                WHERE {conditions}
                ORDER BY timestamp DESC
                LIMIT ?
            ...
            sessions = [
                {
                    'id': row[0], 'task': row[1], 'timestamp': row[2],
                    'result': row[3], 'duration': row[4],
                    'memory_count': row[5] if len(row) > 5 else 0,
                }
                for row in c.fetchall()
            ]
    ...
    def get_stats(self) -> dict:
        ...
        try:
            ...
            type_counts = dict(c.fetchall())
            c.execute("SELECT AVG(importance), AVG(access_count) FROM memories")
            avg = c.fetchone()
            stats = {
                "total_memories": memory_count,
                "total_sessions": session_count,
                "memory_types": type_counts,
                "avg_importance": avg[0] or 0,
                "avg_access_count": avg[1] or 0,
                "fts_available": self.fts_available,
            }
        except Exception as e:
            print(f"Error getting stats: {e}")
            stats = {
                "total_memories": 0, "total_sessions": 0,
                "memory_types": {}, "avg_importance": 0,
                "avg_access_count": 0, "fts_available": self.fts_available,
            }
        conn.close()
        return stats
    ...
    def _row_to_node(row) -> MemoryNode:
        return MemoryNode(
            ...
            last_accessed=row[5], connections=json.loads(row[6]),
            importance=row[7], access_count=row[8],
        )
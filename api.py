...
# async def health():
#     return {
#         "status": "ok", "service": "gotham-orbital", "version": "3.9.0",
#         "ts": utcnow(), "satellites": len(SAT_CATALOG), "db": DB_PATH,
#         "tavily": bool(TAVILY_KEY), "groq_env": bool(os.getenv("GROQ_API_KEY")),
#     }
...
#             "producing confident-sounding conclusions from weak data.\n\n"
#             "Start with [ANALYST-1] SYNTHESIS.\n\n"
#             "MANDATORY SEARCH GATES — treat these identically to hard preconditions.\n"
#             "You MUST complete ALL four searches before writing a single word of output.\n"
#             "If a gate is not satisfied, perform the search NOW — do not skip it.\n\n"
#             "  [SEARCH GATE 1 — CONFLICT ZONES]\n"
#             "    Have you searched for current active conflict zones in EVERY region\n"
#             "    label present in the satellite snapshot (e.g. UKRAINE/BLACK SEA, MIDEAST)?\n"
#             "    If not → search 'active conflict zones <REGION> 2026' right now.\n\n"
#             "  [SEARCH GATE 2 — SATELLITE MISSIONS]\n"
#             "    Have you searched for the known mission and ISR role of EACH named satellite\n"
#             "    (e.g. 'COSMOS-2543 satellite mission', 'YAOGAN-30 reconnaissance role')?\n"
#             "    If not → search for each one right now.\n\n"
#             "  [SEARCH GATE 3 — GROUND ACTIVITY]\n"
#             "    Have you searched for the latest military/ground activity in each overflown\n"
#             "    region (e.g. 'Ukraine frontline situation March 2026',\n"
#             "    'Middle East military activity March 2026')?\n"
#             "    If not → search for each region right now.\n\n"
#             "  [SEARCH GATE 4 — COORDINATION EVIDENCE]\n"
#             "    Have you searched for any open-source evidence of Russia-China satellite\n"
#             "    coordination or joint ISR activity (e.g. 'Russia China military satellite\n"
#             "    cooperation 2025 2026')?\n"
#             "    If not → search for it right now.\n\n"
#             "  [UNKNOWN] IS ONLY VALID AFTER A SEARCH RETURNED NO RESULT.\n"
#             "  Writing [UNKNOWN] without having searched is an analytical failure.\n"
#             "  The DATA GAPS section must list what you searched for and why it was insufficient\n"
#             "  — not what you assumed was unknowable without trying.\n\n"
#             "STRICT OUTPUT RULES:\n"
#             "  1. Tag every factual claim as [RETRIEVED], [INFERRED], or [UNKNOWN].\n"
#             "  2. IF-THEN-RESULT chains may only use [RETRIEVED] or [INFERRED] claims.\n"
#             "     Any chain built on [UNKNOWN] must be in SPECULATIVE section only.\n"
#             "  3. CONFIDENCE must reflect the evidence actually retrieved. If you have 3 or more\n"
#                 "     [RETRIEVED] claims in ASSESSMENT, confidence must be at least 60%.\n"
#                 "     Underrating confidence when evidence exists is as wrong as overrating it.\n"
#             "  4. RECOMMENDATION rules:\n"
#             "       - Must name a specific satellite ID from the catalog\n"
#             "       - Must name a specific region or geographic bounding box\n"
#             "       - Must name a specific observable indicator to watch\n"
#             "       - 'Monitor the situation' and satellite names not in the catalog are INVALID\n\n"
#             "Format:\n"
#             "  SYNTHESIS         — 2 sentences: what is known from satellite data + search\n"
#             "  ASSESSMENT        — IF/THEN/RESULT using [RETRIEVED]/[INFERRED] only\n"
#             "  SPECULATIVE       — IF/THEN/RESULT for [UNKNOWN]-based chains only\n"
#             "  DATA GAPS         — what you searched for but could not retrieve\n"
#             "  CONFIDENCE        — percentage + which retrieved claims justify it\n"
#             "  RECOMMENDATION    — satellite ID + region + specific indicator\n\n"
#             "MANDATORY FINAL LINE:\n"
#             "RELEVANT OBJECTS: [comma-separated IDs from: "
#             "ISS, TIANGONG, NOAA19, TERRA, AQUA, SENTINEL2B, STARLINK30, STARLINK31, "
#             "IRIDIUM140, GPS001, GLONASS, COSMOS2543, YAOGAN30, LACROSSE5]\n"
#             "Include every satellite ID mentioned anywhere in your response. "
#             "Do not omit this line."
...
"""
Gotham Orbital — Fused Intelligence Backend  v4.0
==================================================
Fixes applied in this version
------------------------------
1.  workers=1          — _agent/_agent_groq_key are process globals; workers>1
                         splits them across processes so only one worker gets the key.
2.  max_steps=6        — was 20; each step = 1 Groq call; 20 steps burns Groq's
                         12k token/min budget in a single request.
3.  Semaphore(2)       — caps concurrent Groq calls across the whole process.
4.  Retry back-off     — moved into llm.py (15 s / 30 s / 60 s, max 3 attempts).
5.  Task truncation    — fused_task capped at 6 000 chars; prevents Groq 400 errors
                         that cause the "JSON parse failed → web_search with full
                         prompt → query too long" cascade.
6.  Staggered TLE fetch— 200 ms gap between Celestrak requests; prevents IP bans.
7.  x-llm-model header — frontend can override the Groq model per-request.
8.  GET /models        — returns available models so the frontend can build a picker.
9.  Default model      — llama-3.1-8b-instant (131k tok/min vs 12k for 70b).
"""
...
TAVILY_KEY   = os.getenv("TAVILY_API_KEY", "")
PROXIMITY_KM = 500
MAX_TASK_CHARS = 6000   # hard cap to prevent Groq 400 errors

os.makedirs(os.path.dirname(DB_PATH) if os.path.dirname(DB_PATH) else ".", exist_ok=True)

# ── Groq concurrency guard ────────────────────────────────────────────────────
# Caps simultaneous Groq calls across the whole process.
# Without this, parallel frontend requests multiply 429 pressure.
...
AVAILABLE_MODELS = [
    {
        ...
        "tokens_min":  131072,
        "recommended": True,
        "note":        "Best for multi-step agents — highest token budget",
    },
    {
        ...
        "tokens_min":  12000,
        "recommended": False,
        "note":        "Highest quality but hits rate limits fast",
    },
    {
        "id":          "llama3-70b-8192",
        ...
        "tokens_min":  6000,
        "recommended": False,
        "note":        "Lower rate limit than 3.3",
    },
    {
        "id":          "gemma2-9b-it",
        "label":       "Gemma 2 9B IT",
        "tokens_min":  15000,
        "recommended": False,
        "note":        "Good balance of quality and limits",
    },
    {
        "id":          "mixtral-8x7b-32768",
        "label":       "Mixtral 8×7B 32768",
        "tokens_min":  5000,
        "recommended": False,
        "note":        "Lowest rate limit — avoid for agents",
    },
]
...
async def fetch_all_tles() -> dict:
    """Fetch TLEs with 200 ms stagger to avoid Celestrak IP-banning the EC2 instance."""
    results = {}
    now = datetime.now(timezone.utc)
    for sat_id, norad_id in NORAD_IDS.items():
        ...
        await asyncio.sleep(0.2)   # ← stagger: prevents Celestrak rate-limit
    return results
...
async def get_agent(groq_key: str = "", tavily_key: str = "", model: str = "") -> Any:
    global _agent, _agent_groq_key, _agent_model
    ...
    async with _lock:
        key_changed   = effective_groq  != _agent_groq_key
        model_changed = effective_model != _agent_model

        if _agent is None or key_changed or model_changed:
            log.info(
                f"Building EZAgent — model: {effective_model} "
                f"key: {effective_groq[:8]}..."
            )
            os.environ["GROQ_API_KEY"]   = effective_groq
            os.environ["TAVILY_API_KEY"] = effective_tavily
            _agent = None

            # Update the shared LLM client's api_key so it authenticates correctly.
            # We do NOT set llm_client.model here — the model is passed explicitly
            # into EZAgent and flows through every LLM call via model=self.model.
            # This means two concurrent requests with different models don't stomp
            # on each other.
            ...
            from AgenT import EZAgent as _EZAgent
            loop = asyncio.get_event_loop()
            # Pass model explicitly — EZAgent → FastAgent → every llm.simple_prompt call
            _agent = await loop.run_in_executor(
                None, lambda: _EZAgent(DB_PATH, model=effective_model)
            )
            _agent_groq_key = effective_groq
            _agent_model    = effective_model
            log.info(f"EZAgent ready — model: {_agent.model}")
...
def truncate_task(text: str, limit: int = MAX_TASK_CHARS) -> str:
    """
    Hard-cap the task string to avoid Groq 400 errors.
    The cascade was: prompt too long → 400 → JSON parse fail → agent calls
    web_search with the ENTIRE prompt as query → Tavily 400 (>400 chars).
    """
    if len(text) <= limit:
        return text
...
async def _ask(agent: Any, task: str, max_steps: int = 6) -> str:
    """
    Run the agent with:
      - Hard task truncation (prevents Groq 400)
      - Global semaphore (prevents concurrent 429 storms)
      - max_steps defaulting to 6 (was 20 — each step = 1 Groq call)
    """
...
async def list_models():
    """Return available Groq models so the frontend can build a model picker."""
    return {
...
async def ingest_snapshot(
    ...
    if not req.snapshot.strip():
        raise HTTPException(400, "snapshot cannot be empty")
    agent  = await get_agent(x_groq_key, x_tavily_key, x_llm_model)
    stored = await store_snapshot(agent, req.snapshot, req.cycle)
    return IngestResponse(stored=stored, cycle=req.cycle, ts=utcnow())
...
async def intel_query(
    ...
    proximity_block = (
        "Proximity alerts:\n" + "\n".join(proximity_alerts)
    ) if proximity_alerts else ""

    fused_task = "\n\n".join(filter(bool, [
        f"You are ATLAS, a satellite intelligence analyst. Query: {req.query}",
...
async def run_agent(
    ...
    ROLE_TASK = {
        "orbital": (
            "You are ORBITAL-1. Analyze satellite positions. "
            "Give a 4-5 bullet brief: satellite name, region, altitude, anomalies. "
            "Start with [ORBITAL-1]."
        ),
        "news": (
            "You are NEWS-1, a geopolitical OSINT analyst. "
            "Give a 3-bullet brief on strategic context. Each bullet needs a verifiable fact. "
            "Start with [NEWS-1]."
        ),
        "analyst": (
            "You are ANALYST-1. Synthesize the intel. Start with [ANALYST-1] SYNTHESIS.\n"
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.getenv("PORT", 8000))
    # workers=1 is mandatory — _agent is a process-global singleton.
    # workers > 1 means each worker builds its own agent and hammers Groq independently.
    uvicorn.run("api:app", host="0.0.0.0", port=port, reload=False, workers=1)